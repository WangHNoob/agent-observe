...
export function Dashboard() {
  ...
  if (!data) return <Empty text="暂无数据" hint="近 24 小时内尚无 trace 写入" />;
  ...
  return (
    <>
      ...
      <div className="stat-grid">
        <Stat
          label={`近 ${HOURS}h Trace`}
          value={String(data.tracesTotal)}
          hint={`ok ${data.tracesOk} · error ${data.tracesError}`}
        />
        ...
        <Stat
          ...
          small
          hint={`in ${fmtTokens(data.inputTokens)} / out ${fmtTokens(data.outputTokens)}`}
        />
      ...
      <div className="card">
        ...
        <div className="trend" role="img" aria-label="近 24 小时 trace 数量趋势">
          {data.trend.map((t) => {
            ...
            return (
              <div
                className="col"
                key={t.bucket}
                title={`${t.bucket}Z · ${t.n} 条（${t.errors} 错误）`}
              >
                <div className="bar" style={{ height: `${Math.max((t.n / maxN) * 100, 2)}%` }} />
                {t.errors > 0 ? (
                  <div className="bar err" style={{ height: `${Math.max((t.errors / maxN) * 100, 2)}%` }} />
                ) : null}
      ...
      <div className="two-col">
        ...
        <div className="card">
          ...
          {data.recentErrors.length === 0 ? (
            <Empty text="近 24h 无错误" hint="一切正常" />
          ) : (
            <table className="data">
              <thead>
                <tr>
              ...
              <tbody>
                {data.recentErrors.map((t) => (
                  ...
                  >
                    ...
                    <td>
                      <Link to={`/traces/${t.id}`} onClick={(e) => e.stopPropagation()}>
                        {t.name}
                      </Link>
                    </td>
...
function Stat({
  ...
  return (
    <div className="stat">
      ...
      <div className={`value${small ? " small" : ""}${danger ? " danger" : ""}`}>{value}</div>
